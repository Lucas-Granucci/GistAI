...
def process_and_store_articles(articles_to_process: dict):
    ...
    for _, article_data in tqdm(articles_to_process.items()):
        ...
        c.execute(
            "INSERT INTO articles (title, author, publishedAt, url, urlToImage, content, extracted_content, summary, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)",
            (
                article_data["title"],
                article_data["author"],
                ...
                article_data["extracted_content"],
