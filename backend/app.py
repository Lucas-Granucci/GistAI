...
from backend.news.process_news import process_and_store_articles, get_unprocessed_articles
...
async def get_voiceover_link(request_url: str, script_response: Script) -> str:
    async with aiosqlite.connect("backend/database/voice_recordings.db") as conn:
        ...
        await c.execute("SELECT voiceover_link, created_at FROM voice_recordings WHERE request_url = ?", (request_url,))
        ...
        voiceover_response = await generate_voiceover(Script(content=script_response.content))
        new_link = voiceover_response.speech_data["SynthesisTask"]["OutputUri"]

        if result:
            await c.execute("UPDATE voice_recordings SET voiceover_link = ?, created_at = ? WHERE request_url = ?",
                            (new_link, current_time, request_url))
        else:
            await c.execute("INSERT INTO voice_recordings (request_url, voiceover_link, created_at) VALUES (?, ?, ?)", 
...
async def fetch_and_store_articles(count: Optional[int] = Body(5, embed=True)):
    ...
    try:
        ...
        articles = {
            article_key: {k: v for k, v in article_data.items() if k != "extracted_content"}
...
async def generate_script(article_keys: ArticleKeys):
    ...
    try:
        ...
        if not articles:
            raise HTTPException(status_code=404, detail="No articles found for provided keys")
...
async def generate_deep_dive_script_endpoint(article_title: str):
    ...
    try:
        ...
        if not article:
            raise HTTPException(status_code=404, detail="No article found for the given title")
